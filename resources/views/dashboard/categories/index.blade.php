...
@section('title')
All Categories ({{ \App\Models\Category::count() }})
@endsection
...
<div class="">
    <div class="clearfix"></div>
      <div class="col-md-12 col-sm-12">
        ...
        <div class="x_panel">
          <div class="x_title">
            <h2>All Categories ({{ \App\Models\Category::count() }})</h2>
              ...
              <tbody>
                @forelse ($categories as $category)
                    <tr>
                        ...
                        <td>{{ $category->category_parent_id != null ? $category->subCategory->name : '-' }}</td>
                        <td>{{ $category->create_user->name }}</td>
                        <td>{{ $category->update_user->name ?? '-' }}</td>
                        @if(auth()->user()->user_type == "admin")
                        <td>
                            <div class="d-flex justify-content-center">
                                <a class="btn btn-primary btn-md m-1 px-3" href="{{ route('categories.edit', $category->id) }}"title="Edit ({{ $category->name }})">
                                    <i class="fa fa-edit f-18"></i>
                                </a>
                                <form action="{{ route('categories.destroy', $category->id) }}" method="POST">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" onclick="return confirm('Are you sure that you want to delete ({{ $category->name }}) category?');"title="Delete (<?php echo $category->name; ?>)" class="btn btn-danger btn-md m-1 px-3"><i class="fa fa-trash-o f-18"></i></button>
                                </form>
                            </div>
                        </td>
                        @endif
                ...
                @empty
                    <tr>
                        <td colspan="7" class="text-center">There is no categories/sub-categories yet.</td>
                    </tr>
                @endforelse
              </tbody>
            </table>
