...
<div class="">
    ...
    <div class="col-md-12 col-sm-12">
        ...
        <div class="x_panel">
            <div class="x_title">
                <h2>All Users ({{ \App\Models\User::count() }})</h2>
                                ...
                                <tbody>
                                    @forelse ($users as $user)
                                        <tr>
                                            ...
                                            {{-- @forelse (\App\Models\User::with('create_user', 'update_user')->get() as $user) --}}
                                                <td>{{ $user->create_user->name ?? '-' }}</td>
                                                <td>{{ $user->update_user->name ?? '-' }}</td>
                                            {{-- @empty --}}
                                            {{-- @endforelse --}}
                                            ...
                                            @if(auth()->user()->user_type == "admin")
                                                <td>
                                                    <div class="d-flex justify-content-center">
                                                        <a class="btn btn-primary btn-md m-1 px-3" href="{{ route('users.edit', $user->id) }}" title="Edit ({{ $user->name }})">
                                                            <i class="fa fa-edit f-18"></i>
                                                        </a>
                                                        <form action="{{ route('users.destroy', $user->id) }}" method="POST">
                                                            @csrf
                                                            @method('DELETE')
                                                            <button type="submit" onclick="return confirm('Are you sure that you want to delete ({{ $user->name }}) Product?');" title="Delete ({{ $user->name }})" class="btn btn-danger btn-md m-1 px-3">
                                    ...
                                    @empty
                                        <tr>
                                            <td colspan="9" class="text-center">There are no Users yet.</td>
                                        </tr>
                                    @endforelse
                                </tbody>
                            </table>
