                                ...
                                <tbody>
                                    @forelse ($admins as $admin)
                                        <tr>
                                            ...
                                            <td>{{ $admin->create_user->name ?? '-' }}</td>
                                            <td>{{ $admin->update_user->name ?? '-' }}</td>
                                            @if(auth()->user()->user_type == "admin")
                                                <td>
                                                    <div class="d-flex justify-content-center">
                                                        <a class="btn btn-primary btn-md m-1 px-3" href="{{ route('users.edit', $admin->id) }}" title="Edit ({{ $admin->name }})">
                                                            <i class="fa fa-edit f-18"></i>
                                                        </a>
                                                        <form action="{{ route('users.destroy', $admin->id) }}" method="POST">
                                                            @csrf
                                                            @method('DELETE')
                                                            <button type="submit" onclick="return confirm('Are you sure that you want to delete ({{ $admin->name }})?');" title="Delete ({{ $admin->name }})" class="btn btn-danger btn-md m-1 px-3">
                                    ...
                                    @empty
                                        <tr>
                                            <td colspan="9" class="text-center">There are no Admins yet.</td>
                                        </tr>
                                    @endforelse
                                </tbody>
                            </table>
